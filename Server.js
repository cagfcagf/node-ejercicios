...
app.get('/productos', function(req,res,next){  
    ...
    res.send(JSON.parse(fs.readFileSync('./productos.txt')))
})

app.get('/productosRandom', function(req,res,next){
    
    const datosArchivo = JSON.parse(fs.readFileSync('./productos.txt'))
    let arraysId = datosArchivo.map(a => a.id);
    const randomId = Math.floor(Math.random() * arraysId.length);
    res.send(datosArchivo[randomId])
})
